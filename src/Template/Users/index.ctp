		...
		[
			...
			[  
				...
				$this->Html->link(
					...
					['action' => 'view', $user->id],
					['escape' => false, 'class' => 'btn btn-outline-dark btn-sm text-left']
				) . 
				$this->Html->link(
					$this->Pretty->iconUnpaid($user->username) . "Payroll Hours",
					['controller' => 'payrolls', 'action' => 'user', $user->id],
					['escape' => false, 'class' => 'btn btn-outline-danger btn-sm text-left']
				) . 
				$this->Html->link(
				...
				$this->Html->link(
					...
					['action' => 'edit', $user->id],
					['escape' => false, 'class' => 'btn btn-outline-success btn-sm text-left']
				) .
				$this->Html->link(
					$this->Pretty->iconPerm($user->username) . "Titles",
					['action' => 'roles', $user->id],
					['escape' => false, 'class' => 'btn btn-outline-info btn-sm text-left']
				) .
				$this->Form->postLink(
					$this->Pretty->iconDelete($user->username) . "Remove",
					['action' => 'delete', $user->id],
					['escape' => false, 'confirm' => __('Are you sure you want to delete # {0}?', $user->id), 'class' => 'btn btn-outline-primary btn-sm text-left']
				) .
				'</div>',
